...
The file called `mysql-statestore.yaml` has been predefined. You can find it in directory */workspace/gitpod-dapr* (and in VS Code). It contains this content - which sets up this MySQL instance as a Dapr statestore.
...
Then run a Dapr sidecar in the terminal from the directory that contains file `mysql-statestore.yaml`, as follows:
```
...
Note: if you get a message that suggests that Dapr cannot be found, you may need to define the dapr alias in that terminal session:
